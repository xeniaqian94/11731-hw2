...
OUT_DIR=output-pipeline
TRAIN_DATA=en-de/train.en-de.low.filt
# mkdir -p $OUT_DIR

# *** Train n-gram language model and create an FST
python $SCRIPT_DIR/train-ngram.py $TRAIN_DATA.en $OUT_DIR/ngram-fst.txt                                                                                                     $TRAIN_DATA.en $OUT_DIR/ngram-fst.txt

# *** Implement 1: Train IBM Model 1 and find alignment
python ibmpseudo_ppt_example.py --train_source $TRAIN_DATA.de --train_target $TRAIN_DATA.en --align_output $OUT_DIR/alignment.txt
echo "Finished implement 1 "

# *** Implement 2: Extract and score phrases
python phrase_extract.py --train_source $TRAIN_DATA.de --train_target $TRAIN_DATA.en --align_output $OUT_DIR/alignment.txt --phrase_output $OUT_DIR/phrase.txt
