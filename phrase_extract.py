...
def quasi_consec(tp, A):
    ...
    for j in range(minTP, maxTP):
        if not j in tp:
            if not allow_null_alignment or (allow_null_alignment and not (j in A[0].keys())):
                flag = False
    return flag
...
def phrase_extract(idx, A, e, f, max_n):
    extracted_phrases = []
    ...
    for i1 in range(start, len(e)):
        for i2 in range(i1, min(i1 + max_n, len(e))):
            ...
            if len(tp) != 0 and quasi_consec(tp, A):
                ...
                # if i1<5 and i2<5 and "wissen" in f and "sie" in f:
                # print i1, i2, tp, sp
                if len(sp) != 0 and min(sp) >= i1 and max(
                    ...
                    if (i2 + 1 - i1) <= max_n and (j2 + 1 - j1) <= max_n:
                        ...
                        # if not str(f_phrase+" ||| "+e_phrase) in phrase_pair_set:
                        #     print f_phrase+" ||| "+e_phrase
                        #     phrase_pair_set.add(str(f_phrase+" ||| "+e_phrase))
                        # print f_phrase + " ||| " + e_phrase
...
def align(bitext, align_output, phrase_output, max_n):
    ...
    for idx, (e, f) in enumerate(bitext):
        if True:
            ...
            extracted_phrases_aross_corpus += phrase_extract(idx, A, e, f, max_n)

    f_write = open(phrase_output, "w")
    # print extracted_phrases_aross_corpus[:5]
    counter = Counter(extracted_phrases_aross_corpus)
    e_counter = Counter([pair[0] for pair in extracted_phrases_aross_corpus])
    for (e_phrase, f_phrase) in counter.keys():
        f_write.write(f_phrase + "\t" + e_phrase + "\t" + str(
            -1.0 * np.log(counter[(e_phrase, f_phrase)] * 1.0 / e_counter[e_phrase])) + "\n")
...
if __name__ == '__main__':
    ...
    allow_null_alignment = args.null_alignment

    # print "null alignment " + str(allow_null_alignment)

    if allow_null_alignment:
        from ibmpseudo_ppt_example import read_bitext_file
    else:
        from archive.ibmpseudo_ppt_example_original_direction import read_bitext_file
